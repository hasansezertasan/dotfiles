...
than depending on what each machine happened to accept by hand.

The declaration names each of the six formulae rather than trusting the tap as
a whole. The tap being the repository owner's own is what makes trusting these
formulae reasonable, but it is not a reason to accept whatever the tap gains
later: a formula added upstream stays untrusted until it is listed here, which
keeps the decision an explicit edit.
...
## Checked and dismissed: mise shims do not shadow the Homebrew binaries

Confirming the `shellcheck` install raised a suspicion that mise shims shadow
Homebrew, because the Zsh configuration places the shim directory well ahead of
`/opt/homebrew/bin`:
...
There is no problem. A mise shim for a tool no configuration requests falls
through to the next match on `PATH`, so once the formula is installed the shim
resolves to it:

```console
$ command -v shellcheck
/Users/hasansezertasan/.local/share/mise/shims/shellcheck
$ shellcheck --version | sed -n 2p
version: 0.11.0
```

Forty-three shims are dangling in the sense that mise reports `No version is
set for shim` for them, but none of those names has a Homebrew binary to
shadow, so the intersection that would matter is empty:

```console
$ # for each shim that reports "No version is set", require /opt/homebrew/bin/<name>
$ ...
COUNT: 0
```

An earlier version of this note claimed six shims were dangling and shadowing,
and that the documented local ShellCheck command did not work. Both claims were
wrong. The detection used `--version` as a liveness probe, which is invalid:
`cobo`, `idle3`, and `python3-config` exit non-zero on that flag because they
do not accept it, not because mise refused to resolve them. All three run
correctly through their shims. Adding `shellcheck` to the `Brewfile` fixed the
local command exactly as ADR 0005 intended.
